...
export default function Admins ()  {
    ...
    useEffect(() => {
        const usuarioService = new UsuarioService();
        usuarioService.readAdmins().then(res => {
            if(res){
    ...
    const saveProduct = async() => { 
        ...
        if(usuario.userName.trim()){
            ...
            if(validacion === null){
                ...
                if (usuario.nombre.trim() && usuario.apellido.trim() && usuario.telefono.trim() && usuario.password.trim() 
                && usuario.estado.trim() && password2.trim() && confirmPass === true ) {
    ...
    const header = (
        <div className="table-header">
            <h5 className="p-m-0"><b>Administracion de Admins</b></h5>
